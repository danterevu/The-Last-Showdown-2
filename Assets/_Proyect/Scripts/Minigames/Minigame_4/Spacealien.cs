...
public class SpaceAlien : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        originalExploreSpeed = exploreSpeed;
    ...
    /// Llamado por proyectiles, power ups, etc.
    /// killerPlayer = 1 o 2. Pasar 0 si no hay responsable (no da puntos).
    public void TakeDamage(int killerPlayer)
    {
        Die(killerPlayer);
    }

    /// Empuja al alien en una direccion (usado por Repulsion).
    ...
    /// Aplica slow temporal al alien (usado por SlowField).
    ...
    private IEnumerator SlowRoutine(float multiplier, float duration)
    {
        ...
        isSlowed = true;
        exploreSpeed = originalExploreSpeed * multiplier;
        chaseSpeed = originalChaseSpeed * multiplier;

        yield return new WaitForSeconds(duration);

        exploreSpeed = originalExploreSpeed;
    ...
    private void UpdateChasing()
    {
        ...
        if (chasedPlayer == null) { StartInvestigating(lastSeenPosition); return; }
    ...
    private void UpdateInvestigating()
    {
        ...
        if (toTarget.magnitude > 0.3f) targetDirection = toTarget.normalized;

        Transform player = GetNearestPlayerInRange();
        if (player != null) { chasedPlayer = player; state = AlienState.Chasing; return; }

        if (investigateTimer <= 0f) { state = AlienState.Exploring; PickNewExploreDirection(); }
    }
    ...
    private void PickNewExploreDirection()
    {
        ...
        float newAngle = (currentAngle + Random.Range(-80f, 80f)) * Mathf.Deg2Rad;
    ...
    private void HandleObstacleAvoidance()
    {
        ...
        Vector2 evadeDir;
        if (centerHit && leftHit && rightHit) evadeDir = -moveDirection;
        else if (centerHit) evadeDir = rightHit ? leftDir : rightDir;
        else if (leftHit) evadeDir = rightDir;
        else evadeDir = leftDir;
    ...
    private Vector2 GetBorderAvoidanceDirection()
    {
        ...
        foreach (var hit in hits)
        {
            ...
            pushDir += ((Vector2)transform.position - (Vector2)hit.ClosestPoint(transform.position)).normalized;
            count++;
        }

        return count == 0 ? Vector2.zero : (pushDir / count).normalized;
    }
    ...
    private void Die(int killerPlayer)
    {
        ...
        // Solo da puntos si hay un responsable claro
        if (killerPlayer > 0)
