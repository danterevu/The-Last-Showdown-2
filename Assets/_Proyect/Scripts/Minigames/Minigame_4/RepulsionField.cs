...
public class RepulsionField : MonoBehaviour
{
    ...
    [Header("Alien Knockback")]
    [Tooltip("Fuerza de empuje aplicada a los aliens dentro del radio")]
    [SerializeField] private float alienKnockbackForce = 8f;
    ...
    private void Awake()
    {
        if (circleRenderer == null) circleRenderer = GetComponent<LineRenderer>();
    }
    ...
    {
        if (!Application.isPlaying) SetupCircle();
    }
    ...
    void ApplyRepulsion()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (dist < radius) { float falloff = 1f - (dist / radius); particles[i].velocity += dir.normalized * force * falloff; }
            if (dist > radius) particles[i].remainingLifetime = 0;
        }

        fx.SetParticles(particles, count);
        DestroyProjectilesAndRepelAliensInRadius(center);
    }

    private void DestroyProjectilesAndRepelAliensInRadius(Vector3 center)
    {
        ...
        foreach (Collider2D hit in hits)
        {
            ...
            if (alien != null)
            {
                ...
                alien.ApplyKnockback(pushDir * alienKnockbackForce);
                continue;
            }

            Projectile projectile = hit.GetComponent<Projectile>();
            if (projectile != null) { Destroy(projectile.gameObject); continue; }

            SlowGrandeProjectile slowGrande = hit.GetComponent<SlowGrandeProjectile>();
            if (slowGrande != null) { Destroy(slowGrande.gameObject); continue; }

            HomingMissile homingMissile = hit.GetComponent<HomingMissile>();
            if (homingMissile != null) { Destroy(homingMissile.gameObject); }
        }
    }
}