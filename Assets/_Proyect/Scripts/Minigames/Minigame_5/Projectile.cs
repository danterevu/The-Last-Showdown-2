...
public class Projectile : MonoBehaviour
{
    ...
    private float range;
    private int ownerPlayer;   // 1 o 2, para no da�arse a s� mismo
    ...
    /// <summary>
    /// Llamado por WeaponController justo despu�s de Instantiate.
    /// </summary>
    ...
    {
        Debug.Log($"Colisi�n con: {other.gameObject.name} | tag: {other.tag} | layer: {LayerMask.LayerToName(other.gameObject.layer)}");
        // Determinar si impact� a un jugador
        int hitPlayer = 0;
        ...
        else
        {
            ...
            // Impact� contra una pared u otro objeto
            SpawnHitVfx();
            Destroy(gameObject);
            return;
        }

        // No da�arse a s� mismo
        ...
        // Restar puntos al jugador impactado (da�o directo, sin modificador)
        SpaceMinigame.Instance?.RegisterKill(ownerPlayer, hitPlayer);

        // Sumar puntos al due�o del proyectil (kill bonus con posible multiplicador)
